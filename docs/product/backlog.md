...
|---|---|---|---|---|---|---|
| B15 | **B1 fix batch after usability trial 1 FAILED**: planning floor becomes a frontier band (max − 8, visible + adjustable control per D18) instead of p75-of-catalog; pair distinctness per D19 (≥1.5× price or ≥2.0 intelligence separation between pair members); re-run the usability check as trial 2. | Solo agent user | Trial 2 passes: user gets a trustworthy pair (<60s, planning slot within the frontier band, no mirror-pairs); the DeepSeek-twin case cannot recur (test fixed); frontier-band control renders and re-ranks live; `npm test` green. | 100×3×0.9÷2 = 135 | **Ready** — amendments in `b1-task-brief.md` (§floors, §candidate pairs) + D18/D19 | B12 (done) |
| B1 | Replace single-model picks with a **recommended pair** (planning model + execution model) as the primary output, ranked by expected $ across the default task mix (~65/30/5), with the mix exposed as a visible adjustable parameter from v1. **Params-free data model per D10**: remove `params_b`/`tier` fields, derivation functions, bubble-size encoding, max-params filter, and "Best small" pick (which supersedes B9's null-handling bug by deleting the logic). | Solo agent user | Pair recommendation renders on the page; user can complete a "pick my pair" task in <60s in a usability check (≥5 of 5 internal trials); expected-$ math uses the B2 taxonomy; zero `params_b` references remain in output or UI. | 100×3×0.8÷8 = 30 | **Usability trial 1 FAILED 2026-08-30** — impl verified, but the recommendation was untrustworthy (rank-24 model in planning slot; twin mirror-pairs in top-3). Fix batch = B14 (scores) + B15 (floor/distinctness). Re-run as trial 2 after both land. | B14, B18 |
